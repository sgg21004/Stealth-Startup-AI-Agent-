...
swift run GalaxyLabs skills add \
...
swift run GalaxyLabs skills retain-check \
  --trigger "Amazon Subscribe & Save" \
  --trigger "Expense form"

echo
echo "== 4) Fresh process session still retrieves A =="
swift run GalaxyLabs session --assume-app Safari | tee /tmp/galaxy-retention-session.txt
...
swift run GalaxyLabs session --assume-app Safari | tee /tmp/galaxy-skill-driven.txt
